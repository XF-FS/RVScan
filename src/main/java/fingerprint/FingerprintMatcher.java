...
public class FingerprintMatcher {
    ...
    public List<MatchResult> identifyFingerprintWithDetails(IHttpRequestResponse requestResponse) {
        ...
        // 检查响应大小，如果超过5MB就跳过指纹检测
        final int MAX_RESPONSE_SIZE = 5 * 1024 * 1024; // 5MB
        if (requestResponse.getResponse().length > MAX_RESPONSE_SIZE) {
            if (burp != null && burp.call != null) {
                burp.call.printOutput("[Fingerprint] 响应大小超过5MB (" + requestResponse.getResponse().length + " bytes)，跳过指纹检测");
            }
            return identifiedCMS;
        }
        ...
        for (FingerPrint fingerprint : fingerprints) {
            ...
            if (matchResult != null) {
                ...
                if (!alreadyExists) {
                    ...
                    matchCount++;
                    if (burp != null) {
                        burp.call.printOutput("[Fingerprint] Matched fingerprint: " + matchResult.toString());
                    }
                }
            }
        }
    ...
    private String detectEncodingAndDecode(byte[] bodyBytes, IResponseInfo responseInfo) {
        ...
        if (charset != null) {
            try {
                ...
                if (burp != null && burp.call != null) {
                    //burp.call.printOutput("[Debug] 使用头部指定编码 " + charset + " 解码成功");
                }
                return decoded;
            } catch (Exception e) {
                if (burp != null && burp.call != null) {
        ...
        if (htmlCharset != null) {
            try {
                ...
                if (burp != null && burp.call != null) {
                    burp.call.printOutput("[Debug] 使用HTML指定编码 " + htmlCharset + " 解码成功");
                }
                return decoded;
            } catch (Exception e) {
                if (burp != null && burp.call != null) {
        ...
        for (String testCharset : commonCharsets)
            try {
                ...
                if (testCharset.equals("UTF-8")) {
                    if (burp != null && burp.call != null) {
                        burp.call.printOutput("[Debug] 使用UTF-8编码解码");
                    }
                    return decoded; // UTF-8作为默认首选
                }
            } catch (Exception e) {
                if (burp != null && burp.call != null) {
                    burp.call.printOutput("[Debug] 使用编码 " + testCharset + " 解码失败");
                }
                continue;
            }
        ...
        try {
            ...
            if (burp != null && burp.call != null) {
                burp.call.printOutput("[Debug] 使用默认编码解码");
            }
            return decoded;
        } catch (Exception e) {
            if (burp != null && burp.call != null) {
                burp.call.printError("[Debug] 使用默认编码解码失败");
            }
            return ""; // 返回空字符串
        }
    }
    ...
    private String extractCharsetFromHtml(byte[] bodyBytes) {
        try {
            ...
            if (matcher.find()) {
                ...
                // 增加调试日志
                if (burp != null && burp.call != null) {
                    burp.call.printOutput("[Debug] 从HTML meta标签检测到编码: " + detectedCharset);
                }
                return detectedCharset;
            }
        } catch (Exception e) {
            if (burp != null && burp.call != null) {
                burp.call.printError("[Debug] 从HTML meta标签提取编码失败: " + e.getMessage());
            }
        }
        return null;
    }
    ...
    private String extractFaviconUrl(String html) {
        try {
            ...
            if (matcher.find()) {
                ...
                if (burp != null) {
                    burp.call.printOutput("[Debug] Extracted favicon URL from HTML: " + href);
                }
                return href;
            }
            ...
            while (linkMatcher.find()) {
                ...
                if (linkTag.toLowerCase().contains("rel=\"icon\"") || 
                    ...
                    if (hrefMatcher.find()) {
                        ...
                        if (burp != null) {
                            burp.call.printOutput("[Debug] Extracted favicon URL through step-by-step matching: " + href);
                        }
                        return href;
                    }
                }
            ...
            if (burp != null) {
                burp.call.printOutput("[Debug] Failed to extract favicon URL: " + e.getMessage());
            }
        }
        return null;
    }
    ...
    private boolean matchFaviconHash(List<String> expectedHashes, String actualHash) {
        ...
        for (String expectedHash : expectedHashes) {
            if (expectedHash.trim().equals(actualHash.trim())) {
                // 保留匹配成功的日志
                if (burp != null) {
                    burp.call.printOutput("[Fingerprint] Favicon hash matched successfully: " + actualHash);
                }
                return true;
            }
        }
