...
public class FingerprintConfig {
    ...
    private void deleteSelectedFingerprint() {
        ...
        if (result == JOptionPane.YES_OPTION) {
            ...
            // 自动保存到文件
            try {
                scanner.saveFingerprintsToFile(null);
                JOptionPane.showMessageDialog(configPanel, 
                    "指纹规则删除成功并已保存！",
                    "删除成功", 
                    JOptionPane.INFORMATION_MESSAGE);
    ...
    private static class FingerprintDialog extends JDialog {
        ...
        private void initComponents(FingerPrint existing) {
            ...
            // 匹配方法下拉框 - 支持三种识别方式
            String[] methods = {"keyword", "faviconhash", "regula"};
            methodComboBox = new JComboBox<>(methods);
            methodComboBox.setSelectedIndex(0); // 默认选择keyword
            
            // 匹配位置下拉框 - 提供两个位置
            String[] locations = {"body", "header"};
            locationComboBox = new JComboBox<>(locations);
            locationComboBox.setSelectedIndex(0); // 默认选择body
            ...
            if (existing != null) {
                ...
                // 设置匹配方法下拉框的选中项
                String method = existing.getMethod();
                if (method != null) {
                ...
                // 设置匹配位置下拉框的选中项
                String location = existing.getLocation();
                if (location != null) {
        ...
        private boolean validateInput() {
            ...
            if (methodComboBox.getSelectedItem() == null) {
                JOptionPane.showMessageDialog(this, "请选择匹配方法", "输入错误", JOptionPane.ERROR_MESSAGE);
                return false;
            }
            if (locationComboBox.getSelectedItem() == null) {
                JOptionPane.showMessageDialog(this, "请选择匹配位置", "输入错误", JOptionPane.ERROR_MESSAGE);
                return false;
            }
