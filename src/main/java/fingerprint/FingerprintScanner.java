...
public class FingerprintScanner {
    ...
    private void initializeDefaultFingerprints() {
        try {
            ...
            burp.call.printOutput("[Fingerprint] 已加载 " + fingerprints.size() + " 个指纹");
        } catch (Exception e) {
            burp.call.printError("[Fingerprint] 加载指纹失败: " + e.getMessage());
        }
    }
    ...
    public void updateFingerprintsFromOnline() {
        try {
            burp.call.printOutput("[Fingerprint] 正在从在线源更新指纹库...");
            List<FingerPrint> onlineFingerprints = loader.loadFromUrl(null);
            matcher.loadFingerprints(onlineFingerprints);
            burp.call.printOutput("[Fingerprint] 成功更新指纹库，共 " + onlineFingerprints.size() + " 个指纹");
        } catch (Exception e) {
            burp.call.printError("[Fingerprint] 在线更新指纹库失败: " + e.getMessage());
            burp.call.printError("[Fingerprint] 继续使用当前指纹库");
        }
    }
    ...
    public void loadFingerprintsFromFile(String filePath) {
        try {
            ...
            matcher.loadFingerprints(fingerprints);
            burp.call.printOutput("[Fingerprint] 从文件加载 " + fingerprints.size() + " 个指纹: " + filePath);
        } catch (Exception e) {
            burp.call.printError("[Fingerprint] 从文件加载指纹失败: " + e.getMessage());
        }
    }
    ...
    public void performFingerprintScan(IHttpRequestResponse requestResponse) {
        ...
        try {
            ...
            if (!matchResults.isEmpty()) {
                ...
                burp.call.printOutput("[Fingerprint] " + fullUrl + " 识别到: " + cmsInfo);
            ...
            burp.call.printError("[Fingerprint] 指纹识别过程中发生错误: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void addFingerprintResultWithDetails(IHttpRequestResponse requestResponse, List<FingerprintMatcher.MatchResult> matchResults, String fullUrl) {
        try {
            ...
            if (burp != null && burp.call != null) {
                burp.call.printError("添加指纹识别结果失败: " + e.getMessage());
            }
        }
    }
    ...
    private void addFingerprintResult(IHttpRequestResponse requestResponse, List<String> identifiedCMS, String fullUrl) {
        try {
            ...
            burp.call.printError("[Fingerprint] 添加指纹识别结果失败: " + e.getMessage());
        }
    }
    ...
    public void setEnabled(boolean enabled) {
        ...
        burp.call.printOutput("[Fingerprint] 指纹识别已" + (enabled ? "启用" : "禁用"));
    }
    ...
    public void clearCache() {
        ...
        burp.call.printOutput("[Fingerprint] 指纹识别缓存已清除");
    }
