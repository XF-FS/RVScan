...
public class FingerprintLoader {
    ...
    public List<FingerPrint> loadFingerprints() throws IOException {
        ...
        try {
            ...
            if (defaultFile.exists()) {
                ...
                if (fingerprints != null && !fingerprints.isEmpty()) {
                    burp.call.printOutput("[Fingerprint] 从 " + DEFAULT_LOCAL_FINGERPRINT_PATH + " 加载了 " + fingerprints.size() + " 个指纹");
                    return validateAndFilterFingerprints(fingerprints);
                }
            }
        } catch (Exception e) {
            burp.call.printError("[Fingerprint] 从 " + DEFAULT_LOCAL_FINGERPRINT_PATH + " 加载指纹失败: " + e.getMessage());
        }
        
        // 2. 加载内置的默认指纹
        fingerprints = loadDefaultFingerprints();
        if (fingerprints != null && !fingerprints.isEmpty()) {
            burp.call.printOutput("[Fingerprint] 使用内置指纹库，共 " + fingerprints.size() + " 个指纹");
            return fingerprints;
        }
        ...
        try {
            ...
            if (fingerprints != null && !fingerprints.isEmpty()) {
                burp.call.printOutput("[Fingerprint] 从在线源加载了 " + fingerprints.size() + " 个指纹");
                return fingerprints;
            }
        } catch (Exception e) {
            burp.call.printError("[Fingerprint] 在线加载指纹失败: " + e.getMessage());
        }
    ...
    public List<FingerPrint> loadFromJsonFile(String filePath) throws IOException {
        ...
        if (noStatusCount > 0) {
            burp.call.printOutput("[Fingerprint] " + noStatusCount + " 个指纹没有指定状态码，将接受任何状态码");
        }
