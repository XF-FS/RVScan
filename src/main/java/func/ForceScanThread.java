...
public class ForceScanThread implements Runnable {
    ...
    private void executeScan() {
        ...
        try {
            ...
            // 发送请求
            scanner.getCall().printOutput("[Force Scan Debug] Sending main request for rule: " + name + " to: " + url.toString());
            ...
            if (scanner.getBurp().BypassFirst) {
                scanner.getCall().printOutput("[Force Scan Debug] BypassFirst enabled, sending " + bypassFirstList.size() + " bypass requests");
                performBypassFirst(response, name, rePattern, info, states, rulePath);
            }
            
            if (scanner.getBurp().Bypass) {
                scanner.getCall().printOutput("[Force Scan Debug] Bypass enabled, sending " + bypassList.size() + " bypass requests");
                performBypass(response, name, rePattern, info, states, rulePath);
            }
            
            if (scanner.getBurp().BypassEnd) {
                scanner.getCall().printOutput("[Force Scan Debug] BypassEnd enabled, sending " + bypassEndList.size() + " bypass requests");
    ...
    private void performBypassFirst(IHttpRequestResponse response, String name, String rePattern, 
        ...
        for (String bypass : bypassFirstList) {
            try {
                ...
                scanner.getCall().printOutput("[Force Scan Debug] Sending BypassFirst request with bypass: " + bypass);
