    ...
    private static void go(Map<String, Object> zidian, vulscan vul, IHttpRequestResponse newHttpRequestResponse, List<String> heads, List<String> Bypass_List, List<String> Bypass_List_first, List<String> Bypass_End_List) {
        ...
        if (loaded) {
            ...
            if (states.contains(new Integer(vul.burp.help.analyzeResponse(newHttpRequestResponse.getResponse()).getStatusCode()))) {
                ...
                if (pipe.find()) {
                    synchronized(vul){
                        vulscan.ir_add_with_filter(vul.burp, vul.burp.tags, name, vul.burp.help.analyzeRequest(newHttpRequestResponse).getMethod(), vul.burp.help.analyzeRequest(newHttpRequestResponse).getUrl().toString(), String.valueOf(vul.burp.help.analyzeResponse(newHttpRequestResponse.getResponse()).getStatusCode()) + " ", info, lang, newHttpRequestResponse);
                        IFconform = false;
                    }
                }
            }
            ...
            if (vul.burp.BypassFirst) {
                for (String i : Bypass_List_first) {
                    ...
                    if (bypassResponse.getResponse() != null && states.contains(new Integer(vul.burp.help.analyzeResponse(bypassResponse.getResponse()).getStatusCode()))) {
                        ...
                        if (pipe.find()) {
                            synchronized(vul) {
                                vulscan.ir_add_with_filter(vul.burp, vul.burp.tags, name, vul.burp.help.analyzeRequest(bypassResponse).getMethod(), vul.burp.help.analyzeRequest(bypassResponse).getUrl().toString(), String.valueOf(vul.burp.help.analyzeResponse(bypassResponse.getResponse()).getStatusCode()) + " ", info, lang, bypassResponse);
                            }
                        }
                    }
                }
